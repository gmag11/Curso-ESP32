...
float FilterClass::addWeigth (float coeff) {
    ...
    //DEBUG_VERBOSE (FILTER_TAG, "SumWeight: %f", sumWeight);
...
float FilterClass::aveFilter (float value) {
	...
	DEBUG_VERBOSE (FILTER_TAG, "Value: %f\n", value);

    DEBUG_VERBOSE (FILTER_TAG, "Raw values:");
	for (int i = 0; i < _order; i++) {
        DEBUG_VERBOSE (FILTER_TAG, " %f", _rawValues[i]);
	}

    DEBUG_VERBOSE (FILTER_TAG, "Coeffs:");
	for (int i = 0; i < _order; i++) {
        DEBUG_VERBOSE (FILTER_TAG, " %f", _weightValues[i]);
	}
    ...
    DEBUG_VERBOSE (FILTER_TAG, "Index: %d , left: %d , right: %d\n", _index, left, right);
    ...
    DEBUG_VERBOSE (FILTER_TAG, "Sum: %f", sumValue);
    DEBUG_VERBOSE (FILTER_TAG, " SumWeight: %f\n", sumWeight);

	procValue = sumValue / sumWeight;

    DEBUG_VERBOSE (FILTER_TAG, "Average: %f\n", procValue);
...
float FilterClass::medianFilter (float value) {
	...
	if (_index < _order) {
		...
		right = _order - 1;
		even = ((right - left) % 2) == 1;
        DEBUG_VERBOSE (FILTER_TAG, "%d: ", (right - left) % 2);
		if (even) {
			tempidx = (right - left - 1) / 2;
            DEBUG_VERBOSE (FILTER_TAG, "even\n");
		} 		else {
			tempidx = (right - left) / 2;
            DEBUG_VERBOSE (FILTER_TAG, "odd\n");
		}
    ...
    DEBUG_VERBOSE (FILTER_TAG, "Index: %d , left: %d , right: %d , even: %s , tempidx: %d , medianidx: %d\n", _index, left, right, (even ? "even" : "odd"), tempidx, medianIdx);
    ...
    DEBUG_VERBOSE (FILTER_TAG, "Ordered values:");
	for (int i = 0; i < _order; i++) {
        DEBUG_VERBOSE (FILTER_TAG, " %f", _orderedValues[i]);
	}
    ...
    DEBUG_VERBOSE (FILTER_TAG, "Median: %f\n", procValue);
